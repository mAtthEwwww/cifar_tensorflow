...
def _residual(x, in_filter, out_filter, strides, activate_before_residual, isTrain):
    ...
    if activate_before_residual:
        ...
        with tf.variable_scope('shortcut_path'):
            shortcut_x = x

        with tf.variable_scope('residual_path'):
            x = _batch_norm(
                    name='bn1',
                    x=x,
                    isTrain=isTrain)
...
def inference(
        image,
        num_units=[None, 5, 5, 5],
        num_channels=[16, 16, 32, 64],
        sampling=[False, False, True, True],
        activate_before_residual=[None, True, False, False],
        isTrain=False): 
        ...
        with tf.variable_scope('unit_%d' % unit):
            with tf.variable_scope('block_0'):
                x = _residual(
                        ...
                        activate_before_residual=activate_before_residual[unit],
                        isTrain=isTrain)
